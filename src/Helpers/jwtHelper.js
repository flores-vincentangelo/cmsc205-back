...
let jwtHelper = {
  ...
  getEmployeeEmailFromToken: getEmailFromToken,
};
...
function verifyToken(req, res, next) {
  ...
  if (!token) {
    res.status(401).json({
      ...responses.unathorizedResponseBuilder("Not authorized to access data"),
    });
  } else {
    jwt.verify(token, process.env.SECRET, function (err) {
      if (err) {
        ...
        res.status(401).json({
          ...responses.unathorizedResponseBuilder("Please login again"),
        });
      } else next();
    });
  }
